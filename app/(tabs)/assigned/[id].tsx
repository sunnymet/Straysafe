...
export default function AssignedReportDetail() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const [report, setReport] = useState<Report | null>(null);
  const [loading, setLoading] = useState(true);
  const [address, setAddress] = useState<string>('');

  // Fetch Firestore report
  useEffect(() => {
  ...
  // Reverse‐geocode coords once we have them
  useEffect(() => {
    if (!report) return;
    (async () => {
      try {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.value}>
        {report.volunteerName ?? 'Unclaimed'}{' '}
        {report.volunteerContact ? `(${report.volunteerContact})` : ''}
      </Text>
